...
[ -n "$ZYNTHIAN_BASE" ] || ZYNTHIAN_BASE="https://github.com/zynthian"
[ -n "$ZYNTHIAN_BRANCH" ] || ZYNTHIAN_BRANCH=""

[ -n "$ZYNTHIAN_SYS_REPO" ] || ZYNTHIAN_SYS_REPO="${ZYNTHIAN_BASE}/zynthian-sys.git"
[ -n "$ZYNTHIAN_UI_REPO" ] || ZYNTHIAN_UI_REPO="${ZYNTHIAN_BASE}/zynthian-ui.git"
[ -n "$ZYNTHIAN_ZYNCODER_REPO" ] || ZYNTHIAN_ZYNCODER_REPO="${ZYNTHIAN_BASE}/zyncoder.git"
[ -n "$ZYNTHIAN_WEBCONF_REPO" ] || ZYNTHIAN_WEBCONF_REPO="${ZYNTHIAN_BASE}/zynthian-webconf.git"
[ -n "$ZYNTHIAN_DATA_REPO" ] || ZYNTHIAN_DATA_REPO="${ZYNTHIAN_BASE}/zynthian-data.git"
...
if [ -d "${ZYNTHIAN_ZYNCODER_DIR}/.git" ]; then
    ...
    git clone $(has_branch ${ZYNTHIAN_ZYNCODER_BRANCH}) "${ZYNTHIAN_ZYNCODER_REPO}"
fi
...
if [ -d "${ZYNTHIAN_UI_DIR}/.git" ]; then
    ...
    git clone $(has_branch ${ZYNTHIAN_UI_BRANCH}) "${ZYNTHIAN_UI_REPO}"
fi
cd "$ZYNTHIAN_UI_DIR"
...
if [ -d "${ZYNTHIAN_DATA_DIR}/.git" ]; then
	...
	git clone $(has_branch ${ZYNTHIAN_DATA_BRANCH}) "${ZYNTHIAN_DATA_REPO}"
fi
...
if [ -d "${ZYNTHIAN_WEBCONF_DIR}/.git" ]; then
	...
	git clone $(has_branch ${ZYNTHIAN_WEBCONF_BRANCH}) "${ZYNTHIAN_WEBCONF_REPO}"
fi
